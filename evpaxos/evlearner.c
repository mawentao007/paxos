...
struct evlearner
{
	...
	deliver_function delfun;    /* Delivery callback */   //投递的回调函数
	void* delarg;               /* The argument to the delivery callback */  //回调函数参数
	struct event* hole_timer;   /* Timer to check for holes */    //检查空洞的时间事件 
	struct timeval tv;          /* Check for holes every tv units of time */   //检查空洞的时间间隔
	struct peers* acceptors;    /* Connections to acceptors */       //连接acceptor
};
	...
	struct event_base* b)
{
	...
	l->acceptors = peers_new(b, c->acceptors_count);   //返回指向peers结构体的指针，用来记录peer的信息
