...
void
paxos_log(int level, const char* format, va_list ap)     //记录日志
{
	int off;
	char msg[1024];
	struct timeval tv;
...
void
paxos_log_error(const char* format, ...)    //记录错误日志
{
	va_list ap;
	va_start(ap, format);
...
void
paxos_log_info(const char* format, ...)        //记录消息日志
{
	va_list ap;
	va_start(ap, format);
...
void
paxos_log_debug(const char* format, ...)            //记录调试日志
{
	va_list ap;
	va_start(ap, format);
...
int
paxos_quorum(int acceptors)                        //多数派
{
